...
const SESSION_KEY = "social-strip-dismissed";
...
export function MobileSocialStrip() {
  const t = useTranslations("mobileStrip");
  ...
  return (
    <div
      className="sm:hidden fixed bottom-0 left-0 right-0 z-40
                 bg-background/95 backdrop-blur-sm border-t border-brand-gray
                 px-4 py-3 flex items-center gap-3"
    >
      <span className="text-xs text-foreground/60 shrink-0">{t("follow")}</span>
        ...
        {socials.map(({ label, href, icon }) => (
          <a
            ...
            className="flex items-center justify-center w-8 h-8 rounded-full
                       ...
                       hover:bg-brand-yellow hover:text-foreground transition-colors"
          >
            {icon}
          </a>
        ))}
